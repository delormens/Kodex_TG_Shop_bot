...
from kodex.data.config import get_admins, BOT_VERSION, BOT_DESCRIPTION, PATH_DATABASE
from kodex.data.loader import bot
from kodex.keyboards.reply_main import menu_frep
from kodex.services.api_session import AsyncSession
from kodex.services.api_sqlite import (get_settingsx, update_settingsx, get_userx, get_purchasesx, get_all_positionsx,
...
from kodex.utils.const_functions import get_unix, convert_day, get_date, ded


# Уведомление и проверка обновления при запуске бота
async def startup_notify(dp: Dispatcher, kodex_one: AsyncSession):
    if len(get_admins()) >= 1:
...
async def check_update(kodex_one: AsyncSession):
    session = await kodex_one.get_session()
...
async def check_mail(kodex_one: AsyncSession):
    session = await kodex_one.get_session()
...
async def upload_photo(kodex_one: AsyncSession, this_photo):
    session = await kodex_one.get_session()
